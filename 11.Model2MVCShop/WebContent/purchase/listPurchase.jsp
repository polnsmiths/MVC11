...
<script type="text/javascript">
	...
	$(function() {
		...
		$(".ct_list_pop td:nth-child(1)").on("click", function() {
			...
			self.location = "/purchase/getPurchase?tranNo="+tranNo+"&prodNo="+prodNo+"&tranCode="+tranCode;
		}); 
	...
	//////////////////////////////////////	
		...
		$(".ct_list_pop td:nth-child(13)").on("click", function() {
			...
			self.location = "/purchase/updateTranCode?currentPage=${resultPage.currentPage}&"
...
<!-- ///////////////////////////////////////////////////////////
<form name="detailForm" action="/purchase/listPurchase?buyerId=${buyerId}" method="post">
//////////////////////////////////////////////////////// -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	style="margin-top: 10px;">
	...
	<tr>
		...
		<td class="ct_line02"></td>
		<td class="ct_list_b" width="150">구매한 상품</td>
		<td class="ct_line02"></td>
		<td class="ct_list_b" width="70">받는분 성함</td>
		<td class="ct_line02"></td>
		<td class="ct_list_b">받는분 번호</td>
		<td class="ct_line02"></td>
...
<%-- 		int no=list.size();
	...
	Product product = (Product)listProduct.get(i);//list나 listProduct나 size()개수는 같을테니까..
--%>	
    <c:set var="i" value="0"/> <%--var="i"라는 이름으로 value값 0 저장. 즉, ${i}하면 값인 0이 찍힌다 --%>
    <c:forEach var="purchase" items="${list}" > 
                  <%--items=${list}의 사이즈만큼 돌려서 한개씩var에 저장 => var에 설정한 이름으로 쓰겠다 --%>
  ...
  <%--    ${purchase.tranNo}
     ...
     ${purchase.buyer.userId } --%>
         <%--바로 전에 설정한 i에 value값으로 +1씩 증가하게 하였다. for(i=0; i++)와 같은 의미!!!! 
                                                          참고로 ${i+2}하면 2 4 6 8 이렇게 2씩 증가하는 것을 볼 수 있다 --%>


	<tr class="ct_list_pop">
		<td align="center">
		<!-- /////////////////////////////////////////////
			<a href="/purchase/getPurchase?tranNo=${purchase.tranNo}&prodNo=${purchase.purchaseProd.prodNo}
			&tranCode=${purchase.tranCode}">${i}</a>
		 ///////////////////////////////////////////////////-->
			${i}
		...
		<td></td>
		<td align="left">${state[i-1]}</td>
		<td></td>
		<td align="left">
		<!-- //////////////////////////////////////////////////
		<a href="/purchase/updateTranCode?currentPage=${resultPage.currentPage}&tranNo=${purchase.tranNo}&tranCode=${purchase.tranCode}">${arrived[i-1]}</a>
		 /////////////////////////////////////////////////-->
		${arrived[i-1]}
		<input type="hidden" id="tranNo" value="${purchase.tranNo}"/>
		<input type="hidden" id="tranCode" value="${purchase.tranCode}"/>
